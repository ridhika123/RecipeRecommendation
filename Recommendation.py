...
def get_and_sort_corpus(data):
    '''
    Getting corpus from ingredients_cleaned and sorting each recipe alphabetically.
    '''
    corpus_sorted = []
    for doc in data.cleaned.values:
...
def get_recommendations(N, scores):
    '''
    Generating Top-N recomendations order by score based on cosine similarity.
    '''
    # load in recipe dataset
    df_recipes = pd.read_csv('df_cleaned_final.csv')
    ...
    count = 0
    for i in top:
        recommendation.at[count, "recipe"] = name_parser(df_recipes["recipe_name"][i])
...
def name_parser(name):
    '''
    Converting name of recipe to unicode
    '''
...
def get_recs(ingredients, N=5, mean=True):
    ...
    if model:
        ...
        print(model)
    # load in data
    data = pd.read_csv("df_cleaned_final.csv")
    # parse ingredients
    data["cleaned"] = data.ingredients.apply(ingredient_cleaner)
    # create corpus
